...
| 2026-06-19 | 对话界面改造二期 — Continue + 浮动流式面板 | TBD | MessageBubble 新增 Continue（↘按钮）+ onContinue 回调；ChatViewModel 新增 continueMessage()（追加到已有消息）；新增 StreamingPanel 浮动面板（LED 状态灯/停止/最小化）；ChatScreen Box 叠加流式面板 |
